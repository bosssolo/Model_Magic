...
st.markdown(
    ...
    <p style="
        ...
        margin-bottom: 20px;
        text-align: left;">
        Mindful Machine Models By Siddhant
        
    </p>
    """,
    unsafe_allow_html=True
)
# st.title("Mindful Machine Models By Siddhant")

# --------------- Uploading Section ----------------- # 
uploading_file = st.file_uploader("Upload your CSV file", type=["csv"])

if uploading_file:
    st.success("File uploaded successfully! Let's get to work!")
    ...
    if X_cols and y_col:
        ...
        if model_type == "Linear Regression":
            ...
            st.write(f"R- score: {score * 100:.0f}%" )
            st.write(f"RMSE Score: {score2 * 100:.0f}%" )
            
        elif model_type == "Support Vector Machine":
            accu = accuracy_score(y_test,y_pred)
            st.write(f"Accuracy: {accu * 100:.0f}%")
            ...
            st.write(f"Accuracy: {rop * 100:.1f}%" )

  
        elif model_type == "KNN":
            ac = accuracy_score(y_test,y_pred)
            
            st.write(f"Accuracy: {ac * 100:.0f}%" )
